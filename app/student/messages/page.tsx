...
import { NewDirectConversationButton } from "./new-direct-conversation-button";
    ...
    <DashboardShell role="student">
      <section className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Message your course teachers</p>
        </div>
        <NewDirectConversationButton recipients={recipients} role="student" />
      </section>

      {conversations.length === 0 ? (
        <EmptyState
          title="No conversations yet"
          description="Start a conversation with one of your teachers."
        />
      ) : (
          ...
          {conversations.map((conv) => {
            ...
            return (
              <div key={conv.conversation_id} className="flex items-center hover:bg-slate-50 transition">
                <Link
                  href={`/student/messages/${conv.conversation_id}`}
                ...
                {conv.is_creator && (
                  <div className="pr-4 shrink-0">
                    <DeleteConversationButton conversationId={conv.conversation_id} role="student" />
                  </div>
                )}
              </div>
            );
