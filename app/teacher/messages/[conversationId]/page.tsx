...
export default async function TeacherConversationPage({ params }: Props) {
  ...
  const others = result.participants.filter((p) => p.user_id !== currentUser.id);
  const displayTitle =
    (result.conversationTitle ?? others.map((p) => p.full_name).join(", ")) ||
    "Conversation";
        ...
        <div>
          ...
          {result.conversationType === "group" && (
            <p className="text-xs text-slate-400">
              {result.participants.map((p) => `${p.full_name} (${p.role})`).join(", ")}
            </p>
          )}
        </div>
      </div>
