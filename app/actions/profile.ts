  ...
  try {
    ...
    if (userData.role === "student") {
      const { data: student } = await supabase
        .from("students")
        .select("registration_number, class, section, parent_id, parent_key")
        .eq("id", session.userId)
        .single();
      ...
      if (student) {
        ...
        // Auto-generate parent_key for students who registered before the feature was added
        if (!studentData.parent_key) {
          ...
          await (supabase.from("students") as any)
            ...
            .eq("id", session.userId);
          profile.parent_key = newKey;
        } else {
          profile.parent_key = studentData.parent_key;
        }
      }
