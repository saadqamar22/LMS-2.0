...
export async function GET(request: Request) {
  try {
    ...
    const allowedBuckets = ["assignments", "submissions", "materials"];
    ...
    // For materials: anyone authenticated can view
    if (bucket === "materials") {
      ...
      if (signedUrlError) {
        return NextResponse.json(
          { success: false, error: signedUrlError.message || "Failed to generate file URL." },
          { status: 500 },
        );
      }
